...
# prepare song clusters, 
segclusters = [audio.AudioQuantumList()]*len(centroids)
for s in range(0,len(idx)):
	segment = song.analysis.segments[s]
	cluster = idx[s]
	segment.numpytimbre = array(segment.timbre)
	segclusters[cluster].append(segment)
...
for s in range(0,len(idx)):
	...
	for cluster in range(0,num_clusters):
		if(cluster==cluster1):
			continue # skip the reference cluster 
		if(best_match>0):
